...
<?php header('Content-type: Application/xml; charset="utf8"', true); ?>
		<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9"
			...
			<url>
				...
				<url>
				<loc><?php echo $ayarlar["strURL"]; ?>/projelerimiz</loc>
				<changefreq>always</changefreq>
				<lastmod><?php echo date('c', strtotime(date("Y-m-d H:i:s"))); ?></lastmod>
				<priority>1</priority>
			</url>
...
<?php
		//Markalar
		$list = $db->query("SELECT * FROM kategoriler WHERE kategori_durum = 1 ORDER BY kategori_ust_id ASC");
		foreach($list as $row){
			$strURL = $ayarlar["strURL"].'/kategori/'.$row["kategori_seo"];
?>
			<url>
				<loc><?php echo $strURL ?></loc>
				<lastmod><?php echo date('c', strtotime(date("Y-m-d H:i:s"))); ?></lastmod>
